            ...
            <div class="row mt-5">
                ...
                <div class="col-md-6">
                    <div class="row form-group mt-3">
                        <label class="col-sm-9 text-right" for="rack_a_max">RACK "A" TOTLA WEIGHT MUST LESS THAN:<span
                        ...
                        <div class="col-sm-3 @if ($errors->has('rack_a_max')) has-error @endif">
                            <input type="number" id="rack_a_max" name="rack_a_max" class="form-control"
                                value="{{ old('rack_a_max', empty($setting['rack_a_max']) ? '' : $setting['rack_a_max']) }}"
                                required>
                            @if ($errors->has('rack_a_max'))
                                <br><span class="w-100 ml-2 small error">{{ $errors->first('rack_a_max') }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="row form-group mt-3">
                        <label class="col-sm-9 text-right" for="rack_b_max">RACK "B" TOTLA WEIGHT MUST LESS THAN:<span
                                class="required">*</span></label>
                        <div class="col-sm-3 @if ($errors->has('rack_b_max')) has-error @endif">
                            <input type="number" id="rack_b_max" name="rack_b_max" class="form-control"
                                value="{{ old('rack_b_max', empty($setting['rack_b_max']) ? '' : $setting['rack_b_max']) }}"
                                required>
                            @if ($errors->has('rack_b_max'))
                                <br><span class="w-100 ml-2 small error">{{ $errors->first('rack_b_max') }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="row form-group mt-3">
                        <label class="col-sm-9 text-right" for="orderdate">Complete Order Date:<span
                                class="required">*</span></label>
                        <div class="col-sm-3 @if ($errors->has('orderdate')) has-error @endif">
                            <input type="text" id="orderdate" name="orderdate" class="form-control"
                                value="{{ old('orderdate', empty($setting['orderdate']) ? '' : $setting['orderdate']) }}"
                                required>
                            @if ($errors->has('orderdate'))
                                <br><span class="w-100 ml-2 small error">{{ $errors->first('orderdate') }}</span>
                            @endif
                        </div>
                    </div>
